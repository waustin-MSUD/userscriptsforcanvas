// ==UserScript==
// @name          Banner for Companion Shells
// @version       2026.05.15
// @namespace     CTLD
// @description   Display a banner at the top of Companion Shells
// @author        CTLD
// @updateurl     https://raw.githubusercontent.com/waustin-MSUD/userscriptsforcanvas/refs/heads/main/BannerCompanion.user.js
...
(function () {
  ...
  fetch(`/api/v1/courses/${courseId}`, {
    ...
    .then(res => res.json())
    .then(course => {
      if (!course?.name || !/companion/i.test(course.name)) return;

      // Avoid inserting multiple times
      if (document.getElementById('companion-banner')) return;

      const banner = document.createElement('div');
      banner.id = 'companion-banner';
      banner.textContent = '⚠️ Companion Course Shell ⚠️';
